...
class DocxReader {
    ...
    public function toHtml() {
        if ($this->fileData) {
            ...
            $this->html = '<!doctype html><html><head><meta http-equiv="Content-Type" content="text/html;charset=utf-8" /><title></title><style>span.block { display: block; }</style></head><body>';

            $this->convertDoc($children);
            $this->addEnd();
    ...
    private function convertDoc($children){
        foreach ($children->body->p as $p) {
            ...
            if($p->pPr->pStyle) {
                ...
                //print_r($objectAttrs . " " . "st: " . $objectStyle . " ");
